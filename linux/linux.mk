...
LINUX_POST_BUILD_HOOKS += LINUX_INSTALL_DEBUG_TOOLS

# support phytium_debug.config
ifeq ($(BR2_LINUX_KERNEL_DEBUG_TOOLS_INSTALL_TARGET),y)
define LINUX_INSTALL_DEBUG_TOOLS
	$(LINUX_MAKE_ENV) $(BR2_MAKE) $(LINUX_MAKE_FLAGS) -C $(@D)/tools/perf NO_JEVENTS=1 NO_LIBTRACEEVENT=1
	cp $(@D)/tools/perf/perf $(TARGET_DIR)/usr/bin/
	$(LINUX_MAKE_ENV) $(BR2_MAKE) $(LINUX_MAKE_FLAGS) VMLINUX_BTF="" -C $(@D)/tools/bpf/bpftool
	cp $(@D)/tools/bpf/bpftool/bpftool $(TARGET_DIR)/usr/bin/
endef
endif
...
define LINUX_INSTALL_KERNEL_IMAGE_TO_TARGET
	...
	$(call LINUX_INSTALL_VMLINUX,$(TARGET_DIR)/boot)
endef
endif
